...
测试覆盖高德 POI v5 传输、地点与餐饮标准化、真实餐厅闭环、工具注册、执行策略、会话记忆、路线缓存、路线优化、通勤替换、时间轴优化、约束修复、内容回填、规划模型解析、执行循环收敛、状态跳转压缩、执行基线聚合、固定录制回放、防篡改校验和故障注入。
...
7. **固定验收已有契约样本和 CI，但真实录制仍需持续维护**：仓库内 15 个 `synthetic` 样本验证格式和质量门；仍需在稳定环境定期录制脱敏 `live` Provider 结果评估真实质量。
...
- 使用新增的质量基线持续调整各城市、天数和交通方式的部分接受阈值。
- 录制并固化 15 个固定场景的真实 Provider 结果，持续验证部分完成不会掩盖路线不可用、通勤超限和核心结构错误。
...
- `scripts/run_quality_gate.py` 和 GitHub Actions 会执行编译、全量测试及 15 场景离线回放，缺失或失败场景会使构建失败。
- 完整 Orchestrator 故障恢复验收使用真实运行时、工具注册表、执行策略、SQLite 和确定性优化器，覆盖超时、429、无效 LLM 输出、鉴权失败、SQLite 锁定和路线查询恢复共六类场景。

下一步：

- 在高德和模型配置稳定的隔离环境中生成 15 个 `live` 脱敏样本，用真实录制补充但不替代 `synthetic` 契约样本。
- 继续补充景点不足、长通勤、日程超时，以及路线分段连续失败无法恢复的终止场景。
- 建立真实样本定期刷新、人工脱敏复核和协议版本升级流程。
