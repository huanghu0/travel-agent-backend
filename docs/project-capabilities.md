...
16. 执行确定性语义校验，并使用部分可接受策略检查核心结构、路线、时间轴、通勤和约束质量。
17. 完整校验通过时正常结束；仅剩白名单内非关键问题且质量分达标时带警告结束；否则才把结构化问题交给 LLM 做有限次数修复。
18. 返回行程、会话 ID、实际执行步数、完成模式、质量等级、质量分和未解决警告。
...
7. **固定验收框架已经建立，但录制样本仍需持续填充**：已有五城市 15 个固定场景、录制/回放脚本和确定性质量门；需要定期录制真实 Provider 结果形成稳定回归集。
...
- 固定场景从 2026 年 10 月 12 日开始，避免本阶段开发日期 2026 年 8 月 12 日之前的历史日期。
...
- 新增 `scripts/run_fixed_acceptance_baseline.py`，支持调用本地服务录制 `AgentState`，也支持无外部 API 的离线回放。

下一步：

- 在高德和模型配置稳定的环境中跑完 15 个场景，并将脱敏后的 `AgentState` 录制文件纳入回归测试。
- 增加显式故障注入夹具，固定复现路线失败、景点不足、长通勤和日程超时。
- 在 CI 中把缺失场景或任一核心质量门失败设为构建失败。
